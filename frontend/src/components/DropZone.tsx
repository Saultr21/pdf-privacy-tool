  ...
  return (
    <div className="mx-auto flex w-full max-w-4xl flex-col items-center gap-8 px-6 py-10">
      <div className="text-center">
        <span className="inline-flex items-center gap-1.5 rounded-full border border-slate-200 bg-white px-3 py-1 text-[11px] font-medium uppercase tracking-wider text-slate-600 shadow-soft">
        ...
        <h2 className="mt-4 text-3xl font-semibold tracking-tight text-slate-950 sm:text-4xl">
          ...
          <br />
          <span className="text-slate-500">de forma manual y privada.</span>
        </h2>
      ...
      <div
        ...
        className={cn(
          "group relative flex w-full max-w-2xl cursor-pointer flex-col items-center justify-center overflow-hidden rounded-3xl border-2 border-dashed bg-white px-10 py-16 text-center transition-all duration-200",
          "focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-950 focus-visible:ring-offset-2",
          dragOver
            ? "scale-[1.01] border-slate-950 bg-slate-50 shadow-card"
            : "border-slate-300 hover:border-slate-500 hover:bg-slate-50 hover:shadow-card",
        )}
      >
        <input
          ref={inputRef}
      ...
      {error && (
        <div
          role="alert"
          className="w-full max-w-2xl rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm font-medium text-red-800"
        >
          {error}
        </div>
      )}
...
function Step({
  ...
  return (
    <div className="relative rounded-2xl border border-slate-200 bg-white p-4 shadow-soft">
      <div className="absolute -top-2 left-4 inline-flex h-5 w-5 items-center justify-center rounded-full bg-slate-950 text-[10px] font-bold text-white">
        {n}
      </div>
